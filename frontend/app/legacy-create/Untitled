...
export default function Home() {
  ...
  const [briefColors, setBriefColors] = useState("");
  ...
  const [createdBrief, setCreatedBrief] = useState<DesignBrief | null>(null);
  ...
  async function handleUpload() {
    ...
    try {
      ...
      formData.append("asset_type", "brand_reference");
  ...
  async function handleCreateBrief() {
    if (!createdProject || !briefConcept.trim()) {
      return;
    }
    ...
    if (!apiBaseUrl) {
      setBriefError("API base URL is not configured.");
      return;
    }
    ...
    try {
      const desiredColors = briefColors
        .split(",")
        .map((color) => color.trim())
        .filter(Boolean);
  
      const response = await fetch(
        `${apiBaseUrl}/projects/${createdProject.id}/briefs`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(`Failed to create design brief (${response.status}).`);
      }
      ...
      setBriefError(
        "Unable to create the design brief. Please check the backend and try again.",
      );
    } finally {
  ...
  >
    {submitError}
          ...
          <section className="rounded-lg border border-zinc-200 bg-zinc-50 px-5 py-4 dark:border-zinc-700 dark:bg-zinc-800/50">
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
              Define the creative direction for this design project.
            </p>
            ...
            <div className="mt-5 space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={briefConcept}
                  onChange={(event) => setBriefConcept(event.target.value)}
                  placeholder="e.g. Mediterranean Summer"
                  className="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-50"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={briefColors}
                  onChange={(event) => setBriefColors(event.target.value)}
                  placeholder="sand, ivory, terracotta"
                  className="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-50"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={briefMood}
                  onChange={(event) => setBriefMood(event.target.value)}
                  placeholder="warm, natural, sophisticated"
                  className="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-50"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Avoid strong contrast"
                  className="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-50"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) => setNumberOfConcepts(Number(event.target.value))}
                  className="w-32 rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-zinc-900 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-50"
                />
              </div>
              ...
              <button
                ...
                disabled={!briefConcept.trim() || isCreatingBrief}
                className="rounded-lg bg-zinc-900 px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-zinc-700 disabled:cursor-not-allowed disabled:bg-zinc-300 dark:bg-zinc-100 dark:text-zinc-900"
              >
                {isCreatingBrief ? "Creating brief..." : "Save Design Brief"}
              </button>
              ...
              {createdBrief && (
                <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
                  Design brief created successfully.
                </div>
              )}
            </div>
          </section>
